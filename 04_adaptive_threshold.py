...
CFG = dict(
    ...
    seed = 42,
    data_dir = "./data",
    output_dir = "./output_04_onehot_adaptive_threshold",
    device = "cuda",
)
...
def compute_adaptive_thresholds(probs, class_freq):
    thresholds = np.zeros(NUM_LABELS)
    for i in range(NUM_LABELS):
        quantile = np.clip(1.0 - class_freq[i], 0.5, 0.97)
        thresholds[i] = np.quantile(probs[:, i], quantile)
    return thresholds
...
for epoch in range(CFG["num_epochs"]):
    ...
    macro_f1_05, micro_f1_05, val_probs, val_labels = evaluate(model, val_loader, DEVICE)
    thresholds_ada = compute_adaptive_thresholds(val_probs, class_freq)
    macro_f1_ada, micro_f1_ada, _, _ = evaluate(model, val_loader, DEVICE, thresholds_ada)
    ...
    epoch_log = {
        ...
        "val_macro_f1_ada": macro_f1_ada,
        "val_micro_f1_ada": micro_f1_ada,
    }
    history.append(epoch_log)
    log.info(
        "epoch %d  loss=%.4f  val_macro_05=%.4f  val_macro_ada=%.4f",
        epoch + 1, total_loss / n, macro_f1_05, macro_f1_ada,
    )

    if macro_f1_ada > best_macro_f1:
        best_macro_f1 = macro_f1_ada
        best_thresholds = thresholds_ada.copy()
        torch.save(model.state_dict(), os.path.join(CFG["output_dir"], "best_model.pt"))
        log.info("best model saved (macro-F1_ada=%.4f)", best_macro_f1)

log.info("best val macro-F1 (adaptive): %.4f", best_macro_f1)
...
test_macro, test_micro, test_probs, test_labels = evaluate(model, test_loader, DEVICE, best_thresholds)
test_macro_05, test_micro_05, _, _ = evaluate(model, test_loader, DEVICE)
log.info("Test Macro-F1=%.4f (adaptive)  %.4f (t=0.5)", test_macro, test_macro_05)
...
ax.set_title("TACO one-hot (focal+sampling+adaptive): per-emotion F1")
...
axes[0].legend()
axes[1].plot(hist_df["epoch"], hist_df["val_macro_f1_05"], label="macro (t=0.5)")
axes[1].plot(hist_df["epoch"], hist_df["val_macro_f1_ada"], label="macro (adaptive)")
...
results = {
    "test_macro_f1_adaptive": round(test_macro, 4),
    "test_macro_f1_05": round(test_macro_05, 4),
    "test_micro_f1_adaptive": round(test_micro, 4),
    "best_val_macro_f1_adaptive": round(best_macro_f1, 4),
    "config": CFG,
    "history": history,
}
...
print(f"\nTest Macro-F1 (adaptive)={test_macro:.4f}  (t=0.5)={test_macro_05:.4f}")
