...
CFG = dict(
    ...
    seed = 42,
    data_dir = "./data",
    output_dir = "./output_01_softlabel_baseline",
    device = "cuda",
)
...
train_dataset = GoEmotionsSoftDataset(train_df, tokenizer, CFG["max_length"])
...
def soft_bce_loss(scores, labels_soft):
    """Обычный BCE напрямую на мягких метках (votes/total_raters)."""
    return F.binary_cross_entropy_with_logits(scores, labels_soft)
...
for epoch in range(CFG["num_epochs"]):
    ...
    for batch in train_loader:
        ...
        l_main = soft_bce_loss(scores, lbls)
